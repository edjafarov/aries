...
function DynamicUrlResolver() { //cacheble dynamic low perfomance
    ...
    return {
        bindReq: function (resolved, request, response) {
			var resolvedCopy = {};
			//TODO: clonong of object should be redone
			for (var attr in resolved) {
				if (resolved.hasOwnProperty(attr)) resolvedCopy[attr] = resolved[attr];
			}		
            for(key in resolvedCopy.unusedArguments){
                if(resolvedCopy.params.hasOwnProperty(key)){
        ...
        resolve: function (request, response) {
            ...
            for (configUrl in cachedRegexp) {
                ...
                if (resolveMatch) {
					resolved = {};
					//TODO: clonong of object should be redone
					for (var attr in cachedRegexp[configUrl]) {
						if (cachedRegexp[configUrl].hasOwnProperty(attr)) resolved[attr] = cachedRegexp[configUrl][attr];
					}
