...
var Implementation = {
	statement: {
		...
		tags: {
			...
			'view':{
				arguments: '{instance} with {args} in {template}',
				type: 'single',
				handler: function(tree, content, caller){
					var instance=tree.arguments.instance;
					var args = tree.arguments.args;
					var template=tree.arguments.template;
					return [
						'var k=',instance,'.view("',template,'",{',args,'},true);','_write(k);'
					].join('');
				}
			}
		}
	...
	print: {
		...
		closer: '}',
		handler: function(tree, content, caller) {
			return '_write('+tree.argSource+'.renderView?'+tree.argSource+'.renderView():'+tree.argSource+');';
		}
	},
